...
class AssetServiceLifeUpdateJob < ActivityJob
  ...
  def run
    ...
    rules = PolicyAssetTypeRule.where('class_name LIKE ?', "%#{'And'}%").where.not(policy_id: Policy.find_by(parent_id: nil).id)
    rules.each do |rule|
      assets = Rails.application.config.asset_base_class_name.constantize.joins(asset_subtype: :asset_type).where(organization_id: rule.policy.organization_id, asset_types: {id: rule.asset_type_id})
