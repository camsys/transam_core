...
describe "uploads/_update_events.html.haml", :type => :view do
  it 'info' do
    ...
    test_asset.asset_events.create!(attributes_for(:condition_update_event, :transam_asset => test_asset, :upload => test_upload, :comments => 'test comment 123'))
    test_event = test_asset.asset_events.last
    assign(:upload, test_upload)
    render

    expect(rendered).to have_link(test_event.transam_asset.to_s)
